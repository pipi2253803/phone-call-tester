...
class MainWindow(QMainWindow):
    ...
    def _create_map_tab(self) -> QWidget:
        ...
        if WEB_ENGINE_AVAILABLE:
            ...
            self.map_view.setMinimumHeight(300)
            self.map_view.setPlaceholderText("地图功能需要 PyQt6-WebEngine\n\n请运行: pip install PyQt6-WebEngine folium")
            self.map_view.setStyleSheet(f"""
                QTextEdit {{
    ...
    def _cleanup_old_maps(self, temp_dir: str):
        """清理旧的地图临时文件"""
        if not WEB_ENGINE_AVAILABLE:
            return
    ...
    def _update_location_display(self):
        """更新位置地图显示 - 使用真实地图或文本回退"""
        if not WEB_ENGINE_AVAILABLE:
            # 文本模式显示位置记录
            self._update_location_text_display()
            return
    ...
    def _update_location_text_display(self):
        """文本模式显示位置记录（WebEngine 不可用时使用）"""
        if not self.location_records:
            self.map_view.setText("暂无位置记录")
            return
        
        map_text = "📍 通话位置记录\n"
        map_text += "=" * 50 + "\n\n"
        
        for i, record in enumerate(self.location_records[-15:], 1):
            map_text += f"通话 #{record['call_index']}/{record['total_calls']}\n"
            map_text += f"  号码: {record['phone_number']}\n"
            map_text += f"  时间: {record['timestamp']}\n"
            map_text += f"  位置: {record['location'][:50]}\n"
            coords = self._parse_location_coords(record['location'])
            if coords:
                map_text += f"  坐标: {coords[0]:.4f}, {coords[1]:.4f}\n"
            map_text += "-" * 40 + "\n\n"
        
        if len(self.location_records) > 15:
            map_text += f"... 还有 {len(self.location_records) - 15} 条记录 ...\n"
        
        self.map_view.setText(map_text)
